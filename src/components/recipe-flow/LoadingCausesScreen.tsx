...
// Adjusted to match the structure from 01_loading_feature.html
interface LoadingStepConfig {
  id: string;
  text: (formData: any) => string; // Function to generate text based on formData
  duration: number; // Approximate duration for this step in milliseconds
}

const LoadingCausesScreen: React.FC = () => {
  ...
  const ageCategoryLabel = ageCategories.find(cat => cat.value === formData.ageCategory)?.label || formData.ageCategory;
  const genderLabel = formData.gender === 'male' ? 'Masculino' : formData.gender === 'female' ? 'Feminino' : formData.gender;
  ...
  const loadingStepConfigs: LoadingStepConfig[] = [
    ...
    {
      id: 'send_concern',
      text: (data) => `Fetching User Data (Health Concern: ${data.healthConcern || 'N/A'})`,
      duration: 2500,
    },
    {
      id: 'search_causes',
      text: () => "Initializing Components (Searching Potential Causes)",
      duration: 3000,
    },
    {
      id: 'match_gender_age',
      text: () => `Applying Preferences (Gender: ${genderLabel || 'N/A'}, Age: ${ageCategoryLabel || 'N/A'})`,
      duration: 2000,
    },
    {
      id: 'finalize',
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // TODO: Add a way to stop the loop when the modal is closed or process fully finishes
      // For now, it stops when progress reaches 100 and loading is complete.
      if (isLoadingCompleteRef.current && displayedProgress >= 100) {
        // Optionally, trigger an action upon completion here
      } else {
  ...
  }, [formData]); // Rerun effect if formData changes, though typically loading starts once.
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-300 ease-out font-inter p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-md text-center transform transition-all duration-300 ease-out scale-100 opacity-100">
            ...
            {[...Array(8)].map((_, i) => (
              <div
                ...
                style={{
                  width: `${100 - i * 7.5}%`,
                  height: `${100 - i * 7.5}%`,
                  backgroundColor: `rgba(122, 92, 255, ${0.15 + i * 0.1})`, // --color-purple-base: 122, 92, 255;
        ...
        {/* Step List */} 
        <ul className="space-y-2 mb-6 min-h-[140px]"> {/* min-height to reduce layout shift */} 
          ...
          <div className="bg-gray-200 rounded-full h-2.5 w-full overflow-hidden">
            <div 
              ...
              style={{
                ...
                background: 'linear-gradient(to right, #7a5cff, #f649a3)', // --gradient-start, --gradient-end
              }}
              role="progressbar"
        ...
        <p className="text-xs md:text-sm text-gray-500">
          This process usually takes about {Math.ceil(totalDuration / 1000 / 60)} minute(s). Please keep this window open.
        </p>
      </div>
      {/* CSS for animations (could be moved to globals.css or a style tag in Layout if preferred) */} 
      <style jsx global>{`
        @keyframes pulseRing {
        ...
        .ellipsis::after {
            ...
            content: " 0a0";
            width: 1.5em; /* Adjust width to control space for dots */
            text-align: left;
            vertical-align: bottom;
        }
        @keyframes ellipsis {
            0%   { content: " 0a0"; } /* No dots, ensures space */
            25%  { content: "."; }
            50%  { content: ".."; }
            75%  { content: "..."; }
            100% { content: " 0a0"; } /* Back to no dots */
        }
      `}</style>
    </div>
  );
...
export default LoadingCausesScreen;
